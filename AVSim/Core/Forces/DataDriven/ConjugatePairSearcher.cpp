...
bool ConjugatePairSearcher::fit(const std::vector<DAT>& cloud) {
    m_cloud = &cloud;
    if (cloud.size() >= 5) {
        m_piC = polynomialfit_zero(4, cloud.size(),
                                   [&cloud = cloud](int i) { return 0.5 * (cloud[i].xi[0] + cloud[i].xi[1]); },
        ...
        m_tauC = polynomialfit_zero(5, cloud.size(),
                                    [&cloud = cloud](int i) { return cloud[i].xi[2]; },
...
std::array<double, 3> ConjugatePairSearcher::evaluate(const std::array<double, 3> &x) {
...
std::string ConjugatePairSearcher::pi_points_to_str() {
    ...
    for (int i = 0; i < cloud.size(); ++i){
        ss  << 0.5 * (cloud[i].xi[0] + cloud[i].xi[1]) << ", "
